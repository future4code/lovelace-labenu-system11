...
export default async function changeTeachersTeam(req: Request, res: Response): Promise<any> {
    let errorCode = 400
    ...
    try {
        ...
        const validEmail = await connection('labenu_sys_teachers').where("email", email)
        ...
        if (!validTeam.length) {
            ...
            await connection('labenu_sys_teachers').where("email", email).update('team_id', validTeam[0].id)
